...
try:
    ...
    logging.warning("Google API client not found. OAuth features will be disabled.")
...
class AudioSnippetExtractor:
    ...
    def _get_google_creds(self):
        ...
        # 1. Try loading from environment variable (Base64 encoded pickle)
        if config.GOOGLE_API_TOKEN:
            try:
                import base64
                token_bytes = base64.b64decode(config.GOOGLE_API_TOKEN)
                creds = pickle.loads(token_bytes)
                logger.info("Loaded OAuth credentials from environment variable")
            except Exception as e:
                logger.error(f"Error loading OAuth credentials from environment: {e}")
        
        # 2. Fallback to file
        if not creds and os.path.exists(config.YOUTUBE_API_TOKEN_FILE):
            try:
                with open(config.YOUTUBE_API_TOKEN_FILE, 'rb') as token:
                    creds = pickle.load(token)
                logger.info(f"Loaded OAuth credentials from file: {config.YOUTUBE_API_TOKEN_FILE}")
            except Exception as e:
        ...
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                # If using file, save back to file
                if os.path.exists(config.YOUTUBE_API_TOKEN_FILE):
                    with open(config.YOUTUBE_API_TOKEN_FILE, 'wb') as token:
                        pickle.dump(creds, token)
                        logger.info("Refreshed token saved to file")
            except Exception as e:
                logger.error(f"Error refreshing token: {e}")
                # If refresh fails, we might still be able to use the old one if it's not too old? 
                # No, expired is expired.
                return None
    ...
    async def get_video_info(self, url: str) -> Dict[str, Any]:
        """Extract video information using YouTube Data API or yt-dlp fallback."""
        logger.info(f"Extracting video info for: {url}")
        
        # Try Google Data API first
                ...
                if video_id:
                    def _fetch_api_info():
                        ...
                        if not response['items']:
                            raise ValueError("Video not found via API")
                        ...
                        return {
                            ...
                            'upload_date': snippet.get('publishedAt', '').split('T')[0].replace('-', ''),
                            'thumbnail': snippet.get('thumbnails', {}).get('high', {}).get('url')
                        }

                    loop = asyncio.get_event_loop()
                    return await loop.run_in_executor(None, _fetch_api_info)
            except Exception as e:
                ...
                if not downloaded_file:
                    if request.output_format == 'mp4':
                        ...
                        def _download_audio():
                            ...
                            import glob
                            possible_files = glob.glob(f'{temp_basename}.*')
                            if not possible_files:
                                raise FileNotFoundError(f"Download failed - no file found matching {temp_basename}.*")
